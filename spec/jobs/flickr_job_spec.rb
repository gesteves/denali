...
RSpec.describe FlickrJob, type: :worker do
  ...
  describe '#perform' do
    ...
    it 'enqueues group and album workers with user_id after upload' do
      ...
      expect(FlickrGroupJob.jobs.size).to eq(1)
      expect(FlickrGroupJob.jobs.first['args']).to eq(['123456', 'https://flickr.com/groups/test', user.id])
      expect(FlickrAlbumJob.jobs.size).to eq(1)
      expect(FlickrAlbumJob.jobs.first['args']).to eq(['123456', 'https://flickr.com/albums/test', user.id])
    end
  end
end
