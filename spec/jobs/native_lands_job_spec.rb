...
RSpec.describe NativeLandsJob, type: :worker do
  ...
  describe '#perform' do
    ...
    it 'enqueues CaptionValidityJob after updating territories' do
      response_body = [
        {
          'type' => 'Feature',
          'properties' => {
      ...
      CaptionValidityJob.jobs.clear

      described_class.new.perform(photo.id)

      expect(CaptionValidityJob.jobs.size).to eq(1)
    end
