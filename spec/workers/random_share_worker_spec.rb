...
RSpec.describe RandomShareWorker, type: :worker do
  ...
  describe '#perform' do
    it 'returns early in non-production environment' do
      ...
      expect(BlueskyWorker).not_to receive(:perform_in)
    ...
    context 'with eligible entries' do
      ...
      it 'enqueues BlueskyWorker for Bluesky platform' do
      ...
      it 'enqueues MastodonWorker for Mastodon platform' do
        # Clear any existing jobs
        MastodonWorker.jobs.clear

        # Let real scopes work - old_entry should be eligible
        described_class.new.perform([], ['Mastodon'])

        expect(MastodonWorker.jobs.size).to eq(1)
      end
    end
  end
end
