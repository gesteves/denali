  ...
  def show
    ...
    respond_to do |format|
      format.html {
        ...
        if @photoblog.show_related_entries?
          @related_entries = @entry.related
          @related_srcset = PHOTOS[:entry_list_square][:srcset]
          @related_sizes = PHOTOS[:entry_list_square][:sizes].join(', ')
        end
      }
