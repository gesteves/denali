module MapsHelper
  def tooltip_content(photo, entry)
    content_tag :div, class: 'entry-list__item entry-list__item--map' do
      content_tag :div, class: 'entry-list__wrapper', style: intrinsic_ratio_padding(photo, { square: true }) do
        link_to entry.permalink_url, { class: 'entry-list__link', target: 'blank' } do
