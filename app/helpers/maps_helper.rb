  ...
  def tooltip_content(photo, entry)
    content_tag :div, class: 'entry-list__item entry-list__item--map' do
      content_tag :div, class: 'entry-list__wrapper' do
        link_to entry.permalink_url, { class: 'entry-list__link entry-list__link--photo', target: 'blank', style: "background-color:#{photo.dominant_color.present? ? photo.dominant_color : '#EEEEEE' };" } do
