cache "feed/atom/tagged/#{@tag_slug}/page/#{@page}/count/#{@count}/#{@photoblog.id}/#{@photoblog.updated_at.to_i}" do
  ...
  xml.feed xmlns: 'http://www.w3.org/2005/Atom' do
    if @page.nil? || @page == 1
      xml.id atom_tag(tag_url(tag: @tag_slug), @photoblog.entries.published.first.published_at)
      ...
      xml.id atom_tag(tag_url(tag: @tag_slug, page: @page), @photoblog.entries.published.first.published_at)
    ...
    @entries.each do |e|
      xml.entry do
        xml.id atom_tag(e.permalink_url, e.published_at)
        xml.published e.published_at.utc.strftime('%FT%TZ')
        xml.updated e.published_at.utc.strftime('%FT%TZ')
