  ...
  xml.rss version: '2.0',
    ...
    xml.channel do
      if @page.nil? || @page == 1
        ...
        xml.link tag_url(tag: @tag_slug)
        xml.description @photoblog.plain_description
        xml.tag! 'atom:link', rel: 'self', type: 'application/rss+xml', href: tag_feed_url(tag: @tag_slug, format: 'rss')
      else
        xml.title "#{@tags.first.name.titlecase} - #{@photoblog.name} - Page #{@page}"
        xml.link tag_url(tag: @tag_slug, page: @page)
        xml.description @photoblog.plain_description
        xml.tag! 'atom:link', rel: 'self', type: 'application/rss+xml', href: tag_feed_url(tag: @tag_slug, page: @page, format: 'rss')
      end
      xml.language 'en-us'
      @entries.each do |e|
