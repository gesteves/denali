class Entry < ActiveRecord::Base
  ...
  def related(count = 12)
    Entry.tagged_with(self.tag_list, any: true, order_by_matching_tag_count: true).where('entries.id != ? AND entries.status = ? AND entries.published_at > ?', self.id, 'published', 2.years.ago).limit(count)
  end
